...
test("names the install page from host slug", () => {
  ...
  assert.equal(appNameFromHost("eyesinfo.org"), "護眼學堂");
  assert.equal(appNameFromHost("www.eyesinfo.org"), "護眼學堂");
});
...
test("renders the manifest with the per-app name", () => {
  ...
  assert.equal(manifest.theme_color, "#003153");
  assert.equal(manifest.background_color, "#f3f0e9");
