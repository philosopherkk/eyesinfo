...
export const TOOL_TEXT: L<Record<ToolId, { title: string; blurb: string; canto: string }>> = {
  "zh-Hant": {
    ...
    ask: { title: "問醫生清單", blurb: "教育工具 · 面診前自我準備", canto: "問清楚先好決定" },
    tunnel: { title: "隧道視野示意", blurb: "青光眼周邊視野收窄（不是檢查）", canto: "睇下隧道點樣" },
    haze: { title: "白內障日夜示意", blurb: "顏色變淡、霧、車燈眩光", canto: "夜晚開車矇唔矇" },
    floaters: { title: "飛蚊與簾幕", blurb: "會飄的飛蚊 vs 唔郁的簾幕", canto: "有簾幕去急症；新飛蚊同日散瞳眼底（不是只散瞳便足夠）" },
  ...
  "zh-Hans": {
    ...
    ask: { title: "问医生清单", blurb: "教育工具 · 面诊前自我准备", canto: "问清楚再决定" },
    tunnel: { title: "隧道视野示意", blurb: "青光眼周边视野变窄（不是检查）", canto: "看看隧道怎样" },
    haze: { title: "白内障日夜示意", blurb: "颜色变淡、雾、车灯眩光", canto: "夜晚开车糊不糊" },
    floaters: { title: "飞蚊与帘幕", blurb: "会飘的飞蚊 vs 不动的帘幕", canto: "有帘幕去急诊；新飞蚊当日散瞳眼底（不是只散瞳便足够）" },
  ...
  en: {
    ...
    ask: { title: "Questions for your doctor", blurb: "Education tool · prepare for your visit", canto: "Ask before you decide" },
    ...
    floaters: { title: "Floaters vs curtain", blurb: "Drifting specks vs a fixed shadow", canto: "Curtain → A&E; new floaters → same-day dilated exam (dilation alone is not enough)" },
  ...
  ja: {
    ...
    ask: { title: "医師に聞くリスト", blurb: "教育ツール · 受診前の自己準備", canto: "決める前に確認" },
    tunnel: { title: "トンネル視野の図示", blurb: "緑内障の周辺視野（検査ではない）", canto: "トンネルを見る" },
    haze: { title: "白内障の昼と夜", blurb: "色あせ、かすみ、対向車の眩しさ", canto: "夜の運転" },
    floaters: { title: "飛蚊とカーテン", blurb: "動く飛蚊と動かない影", canto: "カーテンは救急；新しい飛蚊は同日の散瞳眼底（散瞳だけでは足りない）" },
    halo: { title: "夜間ハロー比較", blurb: "単焦点・EDOF・3焦点の図示", canto: "術後保証ではない" },
    warm: { title: "温罨法タイマー", blurb: "示意の計時。医師の指示に従う", canto: "時間を計る" },
...
export const LEGAL_I18N: L<{
  ...
  en: {
    short:
      "This page is public education. It cannot replace an in-person consultation with a registered ophthalmologist and is not a diagnosis. It does not offer booking, sales or referral. Organic conditions affecting sight and endocrine disease (including diabetes) must be assessed by a registered doctor; an ophthalmologist when needed. Prescription medicines require a doctor’s prescription.",
    topicFooter:
    ...
    funding: "This site is self-funded by Dr Poon Ka Kin.",
    correction:
  ...
  ja: {
    short:
      "本ページは市民教育です。登録眼科専門医の対面診察の代わりにはならず、診断でもありません。予約・販売・紹介は行いません。視力に影響する器質的疾患および内分泌疾患（糖尿病を含む）は登録医師が個別に評価し、必要に応じて眼科専門医が扱います。処方薬は医師の処方箋が必要です。",
    topicFooter:
    ...
    umao:
      "《不良広告（医薬）条例》（第231章）第3条により、附表の疾患の治療または予防のために医薬品・外科用具・療法を使わせるおそれのある広告を出してはなりません。附表には視力に影響する器質的状態（症状緩和のための局所点眼の広告のみ例外）および内分泌疾患（糖尿病を含む）が含まれます。本教育サイトは症状の気づき、受診の目安、医師が話し合うことのある選択肢の種類のみを扱い、特定製品を勧めず、効果・治癒・視力温存を約束しません。",
    ppo:
    ...
    funding: "本サイトは潘家健医師の自己資金で運営しています。",
    correction:
