...
function ToolsHub() {
  ...
  return (
    <div className="px-4 pt-5 pb-8 layout-lg:px-6">
      <h1 className="text-[1.35rem] font-semibold text-navy layout-lg:text-[1.5rem]">{t("toolsTitle")}</h1>
      <p className="mt-2 max-w-prose text-[0.88rem] leading-relaxed text-muted">{t("toolsLead")}</p>
      <div className="mt-4 overflow-hidden rounded-xl border border-line bg-card layout-lg:grid layout-lg:grid-cols-2 layout-lg:gap-2 layout-xl:grid-cols-3 layout-lg:overflow-visible layout-lg:rounded-none layout-lg:border-0 layout-lg:bg-transparent">
        {TOOLS.map((item) => (
...
function ToolRow({
  ...
  const cls =
    "flex items-center gap-3 border-b border-line px-4 py-3.5 no-underline last:border-b-0 layout-lg:rounded-xl layout-lg:border layout-lg:border-line layout-lg:bg-card layout-lg:last:border-b";
  return (
